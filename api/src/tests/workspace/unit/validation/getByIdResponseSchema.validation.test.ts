...
import { getByIdResponseSchema, type SafeWorkspaceDetailResponse } from "../../../../modules/workspace/workspace.schemas.js";
...
void describe('getByIdResponseSchema', () => {
    void it('accept valid payload', async (t) => {
        const validPayload: SafeWorkspaceDetailResponse = {
            id: randomUUID(),
            name: 'Test Workspace',
        ...
        for (const testCase of cases) {
            await t.test(testCase.title, async () => {
                const result = getByIdResponseSchema.safeParse(testCase.payload);
    ...
    void it('rejects invalid fields', async (t) => {
        ...
        const invalidCaseByField: InvalidCasesByField<SafeWorkspaceDetailResponse> = {
    ...
    void it('rejects missing required fields', async (t) => {
        ...
        const cases: Array<{
            title: string;
            payload: unknown;
            missingFields: Array<keyof SafeWorkspaceDetailResponse>;
        }> = createMissingRequiredFieldCases(getByIdResponseSchema, validPayload);
