...
import { safeTaskDetailSchema, type SafeTaskDetail } from "../../../../modules/task/task.schemas.js";
...
import {
    invalidNonArrayValues,
    invalidNonDateValues,
    invalidNonEnumValues,
    invalidNonNumberValues,
    invalidNonStringValues,
    invalidNonUUIDValues,
} from "../../../validationTestValues.js";

const taskId = randomUUID();
const assigneeUserId = randomUUID();

const validPayload: SafeTaskDetail = {
    id: taskId,
...
void describe('safeTaskDetailSchema', () => {
    void it('accepts valid payload', () => {
        const result = safeTaskDetailSchema.safeParse(validPayload);
    ...
    void it('rejects invalid payload type', async (t) => {
        ...
        for (const testCase of cases) {
            await t.test(testCase.title, async () => {
                const result = await safeTaskDetailSchema.safeParseAsync(testCase.payload);
    ...
    void it('rejects invalid fields', async (t) => {
        const invalidCasesByField: InvalidCasesByField<SafeTaskDetail> = {
            ...
            assignees: invalidNonArrayValues,
        };
    ...
    void it('rejects missing required fields', async (t) => {
        ...
        }> = createMissingRequiredFieldCases(safeTaskDetailSchema, validPayload);
