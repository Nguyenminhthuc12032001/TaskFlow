...
export class AuthService {
  ...
  async register(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      await this.authRepo.saveRefreshToken(
        ...
        new Date(Date.now() + ms(env.TTL_REFRESH_TOKEN as ms.StringValue)),
        tx,
      );
  ...
  async changePassword(userId: string, data: ChangePasswordBody): Promise<SafeUserResponse> {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const user = await this.authRepo.updatePassword(
        userId,
        await hashValue(data.newPassword),
        tx,
      );
